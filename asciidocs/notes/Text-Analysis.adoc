...
image::./images/cross-product.jpg[]
...
Ein n-gram ist eine Sammlung von n aufeinander folgener Wörter. Wenn das `n = 2` ist spricht man von einem bigram, wenn `n = 3` ist von einem trigram und so weiter.  
...
Vereinfacht besagt das *Zipsche Gesetz*, dass wenn die Elemente einer Menge nach ihrer Häufigkeit geordnet werden, ist die Wahrscheinlichkeit ihres Auftretens umgekehrt proportional zum Platz auf der Häufigkeitsliste.
