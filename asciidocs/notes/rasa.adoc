...
ifdef::backend-html5,backend-revealjs[image:5_levels_of_ai.png[width=100%]]

https://www.youtube.com/watch?v=-F6h43DRpcU&list=PL75e0qA87dlHQny7z43NduZHPo6qd-cRc&index=2
...
Darunter fällt zum Beispiel das Anlegen von Terminen und anderen Benachrichtigungen, die man dann zu der eingestellten Uhrzeit bekommt.
...
Dies wird sehr oft in Form von FAQs genutzt und auch ein paar kleine Follow-up Fragen können hierbei schon definiert sein.

**Level 3: Contextual AI Assistants **

Dieses Level wird derzeit von Rasa unterstützt. Zusätzlich zu normalen FAQs wird hierbei auch der Kontext beachtet.
Es macht nämlich einen Unterschied, wann, wie und in welcher Situation ein Benutzer etwas gesagt hat.
Bei Contextual Assistants wird der Kontext, also was bereits zuvor gesagt wurde, auch beachtet. Außerdem lenken sie Konversationen
in die gewünschte Richtung und werden mit der Zeit immer besser, wenn man sie trainiert.
...
In der folgenden Grafik sieht man ein Flussdiagramm, welches beschreibt, ob man Supervised Embeddings oder Pretrained Embeddings verwenden soll.

ifdef::backend-html5,backend-revealjs[image:pre-trained-vs-supervised.png[]]

https://www.youtube.com/watch?v=seBN1gMJN0U&list=PL75e0qA87dlHQny7z43NduZHPo6qd-cRc&index=4
...
Die generellen Funktionsweisen von Text-Analysis-Tools stehen https://htl-leonding-project.github.io/2021-da-chatbot/notes/text-analysis#_natural_language_processing_nlp[hier] beschrieben.

Ein Beispiel für eine Pipeline, die bereits von Rasa vorkonfiguriert ist, sieht folgendermaßen aus:
...
Ein `WhitespaceTokenizer` ist eine sehr einfache Art eines Tokenizers. Hierbei wird, wie der Name bereits vermuten lässt, der Satz aufgeteilt in verschiedene Tokens.
Ein Token kann zum Beispiel also ein Wort sein und beim `WhitespaceTokenizer` wird der Satz pro Whitespace also Leerzeichen aufgeteilt.

ifdef::backend-html5,backend-revealjs[image:whitespacetokenizer.png[]]

 https://www.youtube.com/watch?v=ET1k9OrsfYQ&list=PL75e0qA87dlHQny7z43NduZHPo6qd-cRc&index=5
...
Außerdem kann man natürlich auch selber einen Tokenizer schreiben.

TIP: `Tokenizer` sollten generell weit am Anfang in einer Pipeline sein, wenn nicht sogar die erste Komponente.
...
Der `EntityExtractor` nimmt dann diese Tokens als Input und produziert Named Entity Recognition Ergebnisse.

*Regular Expressions*

Regular Expressions können verwendet werden um Patterns eines Strings zu beschreiben und können hier bei dem Beispiel von Chatbots beispielsweise verwendet werden, um Zahlen zu beschreiben.

ifdef::backend-html5,backend-revealjs[image:regular-expression-example.png[]]

 https://www.youtube.com/watch?v=ET1k9OrsfYQ&list=PL75e0qA87dlHQny7z43NduZHPo6qd-cRc&index=5

*Lookup Tables*

Lookup Tables können verwendet werden, wenn eine Entity vordefinierte Werte haben kann. Zum Beispiel kann eine Enitity country 194 verschiedene Länder annehmen.

ifdef::backend-html5,backend-revealjs[image:lookup-table-example.png[]]

 https://www.youtube.com/watch?v=ET1k9OrsfYQ&list=PL75e0qA87dlHQny7z43NduZHPo6qd-cRc&index=5

TIP: Der `RegexFeaturizer` muss vor dem EntityExtractor in der Pipeline platziert werden.

==== CRFEntityExtractor

Eine Möglichkeit um Entites aus der Nachricht des Benutzers zu extrahieren ist über den `CRFEntityExtractor`.

ifdef::backend-html5,backend-revealjs[image:extractor.PNG[]]

CRF steht hierbei für `[underline]##C##onditional [underline]##R##andom [underline]##F##ield`. Dieses Modell lernt, welche Komponenten eines Satzes Entities sind und welche Entities diese sind.

Dies macht der `CRFEntityExtractor`, indem er die Sequenzen der Tokens beobachtet. Es wird also ein Token aus dem Satz herausgepickt, und anschließend wird geschaut, ob die Wörter danach und davor zum Kontext des Worts beitragen, um zu erkennen, ob es sich hierbei um eine Entity handelt.
...
Dies wird auch mit den Wörtern davor und danach gemacht.

ifdef::backend-html5,backend-revealjs[image:crf-entity-extractor.png[]]
...
Außerdem wird noch produziert, wie sicher das Modell war, dass diese Entites korrekt sind, also die sogenannte `Confidence` wird ausgegeben und welches Modell verwendet wurde.

==== DucklingHttpExtractor

Falls man komplexere Entites extrahieren will, wie beispielsweise Postleitzahlen, Daten oder generelle Nummern, so kann man den `DucklingHttpExtractor` verwenden.

`Duckling` ist eine Library, die speziell dafür entworfen wurde, dass Informationen, wie Daten, Telefonnummern, E-Mails oder Postleitzahlen extrahiert werden.

ifdef::backend-html5,backend-revealjs[image:duckling-http-extractor.png[]]
...
Diese können dann von dem Intent-Klassifikations-Modell genutzt werden um Muster zu erkennen und anschließend den korrekten Intent vorherzusagen.

ifdef::backend-html5,backend-revealjs[image:featurizer.png[]]

https://www.youtube.com/watch?v=YxMzz6NF6Zw&list=PL75e0qA87dlEjGAc9j9v3a5h1mxI2Z9fi&index=8

Der `LexicalSyntacticFeaturizer` bekommt als Input einen Token und erstellt dann Features für die Entity Extraktion.
Dabei verwendet er ein sogenanntes Sliding Window, welches über jeden Token in der Nachricht des Benutzers verwendet wird.

https://rasa.com/docs/rasa/components/#lexicalsyntacticfeaturizer/

==== CountVectorsFeaturizer

Der `CountVectorsFeaturizer` erstellt Bag-of-Words. Dabei wird gezählt, wie oft ein bestimmtes Wort aus den Trainingsdaten in der Nachricht des Benutzers vorkommt.
Dies wird dann als Input dem Intent-Classification-Model übergeben.

ifdef::backend-html5,backend-revealjs[image:countvectorsfeaturizer.png[]]

https://www.youtube.com/watch?v=YxMzz6NF6Zw&list=PL75e0qA87dlEjGAc9j9v3a5h1mxI2Z9fi&index=8

Beim `CountVectorsFeaturizer` kann man außerdem angeben, dass statt Wörtern sogenannte n-grams verwendet werden sollen.
Dies macht das Modell in der Regel robuster gegen Typos allerdings erhöht sich damit auch die Zeit, die das Modell zum Trainieren benötigt.
...
Der `EntitySynonymMapper` erwartet als Input einen Extractor der Enity Extractors und liefert als Ausgabe modifizierte Werte für die Entities, die bereits vorher erkannt wurden.
...
Wenn also beispielsweise in einem Satz des Benutzers `New York City` oder `NYC` vorkommt, werden diese beide auf den selben Wert `nyc` gesetzt.
...
https://rasa.com/docs/rasa/components/#entitysynonymmapper

==== ResponseSelector

Ein `Selector` sagt die korrekte Response voraus aus einer Menge von möglichen Responses.

Beim `ResponseSelector` wird als Ausgabe ein Key-Value Paar ausgegeben. Als Key wird hierbei der Retrieval Intent ausgegeben und das Value ist die vorhergesagte Response und die Confidence, mit der diese vorhergesagt wurde.
...
Die Vorhersage dieses `ResponseSelector` wird dann vom Dialogue Manager verwendet um die vorhergesagte Response auszugeben.

Außerdem wird hierbei das gleiche neuronale Netzwerk verwendet, wie beim `DIETClassifier`.
